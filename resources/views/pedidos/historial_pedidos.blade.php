<x-app-layout>
    ...
    <div id="detallesPedido" class="fixed inset-0 flex items-center justify-center bg-gray-500 bg-opacity-50 hidden">
        <div class="bg-white p-6 rounded-lg shadow-lg w-1/2">
            ...
            <button id="cancelarPedido" class="mt-4 bg-red-500 text-white py-2 px-4 rounded hidden">Cancelar</button>
        </div>
    </div>
            ...
            axios.get(`/pedidos/${id}`)
                .then(response => {
                    ...
                    pedido.productos.forEach(producto => {
                        detallesHtml += `<li>${producto.nombre} - Cantidad: ${producto.pivot.cantidad}</li>`;
                    });
                    detallesHtml += '</ul>';
