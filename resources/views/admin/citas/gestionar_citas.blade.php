...
@section('content')
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach ($citas as $cita)
                        <tr class="hover:bg-teal-200 cursor-pointer w-full">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center" data-modal-toggle="edit_cita_modal_{{ $cita->id }}"
                                data-modal-target="edit_cita_modal_{{ $cita->id }}">{{ $cita->id }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-center" data-modal-toggle="edit_cita_modal_{{ $cita->id }}"
                                data-modal-target="edit_cita_modal_{{ $cita->id }}">{{ $cita->user->name }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-center" data-modal-toggle="edit_cita_modal_{{ $cita->id }}"
                                data-modal-target="edit_cita_modal_{{ $cita->id }}">{{ $cita->peluquero->name }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-center" data-modal-toggle="edit_cita_modal_{{ $cita->id }}"
                                data-modal-target="edit_cita_modal_{{ $cita->id }}">{{ $cita->servicio }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-center" data-modal-toggle="edit_cita_modal_{{ $cita->id }}"
                                data-modal-target="edit_cita_modal_{{ $cita->id }}">{{ $cita->fecha }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-center" data-modal-toggle="edit_cita_modal_{{ $cita->id }}"
                                data-modal-target="edit_cita_modal_{{ $cita->id }}">{{ $cita->hora->format('H:i') }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <form action="{{ route('citas.actualizar_estado', ['id' => $cita->id]) }}" method="POST">
                                    ...
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-md rounded-lg focus:ring-teal-500 focus:border-teal-500 block w-full text-center p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-teal-500 dark:focus:border-teal-500">
    ...
    @foreach ($citas as $cita)
        <div id="edit_cita_modal_{{ $cita->id }}" tabindex="-1" aria-hidden="true"
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                        ...
                        <button type="button"
                            ...
                            data-modal-toggle="edit_cita_modal_{{ $cita->id }}">
                    ...
                    <form action="{{ route('citas.update', ['id' => $cita->id]) }}" method="post" class="p-4 md:p-5">
                        ...
                        <div class="grid gap-4 mb-4 grid-cols-2">
                            <div class="col-span-2">
                                ...
                                <input type="text" name="user_id" id="user_id" value="{{ $cita->user_id }}"
                            ...
                            <div class="col-span-2">
                                <label for="peluquero_id"
                                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Peluquero</label>
                                <select name="peluquero_id" id="peluquero_id"
                        ...
                        <button type="button"
                            ...
                            data-modal-hide="edit_cita_modal_{{ $cita->id }}"
                            data-modal-target="confirm_delete_modal_{{ $cita->id }}"
                            data-modal-toggle="confirm_delete_modal_{{ $cita->id }}">
                            Eliminar cita
                        </button>
                    </form>
        ...
        <div id="confirm_delete_modal_{{ $cita->id }}" tabindex="-1" aria-hidden="true"
            class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
            <div class="relative p-4 w-full max-w-md max-h-full">
                <!-- Modal content -->
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="p-4 md:p-5">
                        ...
                        <div class="flex justify-end items-center mt-4">
                            <form action="{{ route('citas.destroy', ['id' => $cita->id]) }}" method="post"
                                class="p-4 md:p-5">
                                @csrf
                                @method('DELETE')
                                <input type="hidden" name="id" value="{{ $cita->id }}"
                                    data-delete-route="{{ route('citas.destroy', ['id' => $cita->id]) }}">
                                <button type="submit"
                                    class="text-white inline-flex items-center bg-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800"
