...
export default function ModalReserva({ servicio, isOpen, onClose, onSuccess, userId, peluqueros }) {
  ...
  const [bloqueos, setBloqueos] = useState([]); // Agregar estado para los bloqueos
    ...
    if (peluqueroId && fecha) {
        ...
        axios.get(url)
            .then(response => {
                ...
                setBloqueos(response.data.bloqueos); // Actualizar estado con bloqueos
            })
        ...
        const getAvailableTimes = () => {
          ...
          const blockedTimes = bloqueos.reduce((times, bloqueo) => {
              return times.concat(bloqueo.horas || []);
          }, []);
